...
export default function Footer() {
  ...
  return (
    <footer style={{
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8">
        
        {/* Left Block: Brand + Description + Socials */}
        <div className="lg:col-span-5 flex flex-col gap-8">
          <Link to="/" className="text-4xl font-bold font-headline tracking-tighter" style={{ color: 'var(--text-heading)' }}>
            JOINZ
          </Link>
          <p className="font-body text-base max-w-md leading-relaxed" style={{ color: 'var(--text-muted)' }}>
            ...
            <nav className="flex flex-col gap-4">
              {[
                ...
                <Link
                  key={link.to}
                  to={link.to}
                  className="text-sm font-medium w-fit transition-all duration-200"
                  style={{ color: 'var(--text-muted)' }}
          ...
          <div className="flex flex-col gap-6">
            ...
            <nav className="flex flex-col gap-4">
              {[
                ...
                { to: '/services/reputation', label: 'Reputation Management' },
                { to: '/services/recovery', label: 'Account Recovery' },
                { to: '/services/username', label: 'Username Claim' },
              ].map(link => (
                <Link
                  key={link.to}
                  to={link.to}
